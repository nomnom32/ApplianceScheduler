...
batt_charge_rate = diff([batt_op(1,1) batt_op]); %positive if discharging, %negative if charging
...
for b=1:t % do for 24 hrs
    ...
    temp = (cons - batt_charge_rate(b) - PV(b)- peak); %overpeak when (cons-PV)<peak
    if temp>0 
...
for b=1:t %Appliance Price Consumption
    cons_price = cons_price + app_hour_energy(b)*price(1,b);
end
